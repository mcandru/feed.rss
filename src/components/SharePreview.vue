...
const selectedUrls = ref<Set<string>>(new Set(sharedFeeds.value.map(f => f.url)))
...
const someSelected = computed(() => 
  selectedUrls.value.size > 0 && selectedUrls.value.size < sharedFeeds.value.length
)
...
function handleImport() {
  const feedsToImport = sharedFeeds.value.filter(f => selectedUrls.value.has(f.url))
...
<template>
  <div class="min-h-screen bg-bg flex flex-col">
    <!-- Error state -->
    <div v-if="shareError" class="flex-1 flex items-center justify-center">
      <div class="text-center py-16 px-8">
        <div class="w-16 h-16 mx-auto mb-6 bg-red-100 rounded-2xl flex items-center justify-center text-red-500">
    ...
    <template v-else>
        ...
        <div class="max-w-[900px] mx-auto py-4 px-8 sm:px-5">
          <div class="flex items-center justify-between gap-4">
            <div class="flex-1 min-w-0">
              <h3 class="font-medium mb-1">Preview Mode: Someone shared their feeds with you</h3>
              <p class="text-sm opacity-90">
                {{ sharedFeeds.length }} feed{{ sharedFeeds.length !== 1 ? 's' : '' }} • 
            ...
            <div class="flex gap-2 shrink-0">
              ...
              <button 
                ...
                :class="{ 'opacity-50 cursor-not-allowed': selectedUrls.size === 0 }"
                @click="handleImport"
              >
                Import {{ selectedUrls.size }}
              </button>
              <button 
                class="px-4 py-2 rounded-lg bg-transparent hover:bg-white/10 transition-colors duration-200 text-sm font-medium"
                @click="handleDecline"
              >
                Decline
              </button>
      ...
      <header class="border-b border-border bg-bg-elevated">
        <div class="max-w-[900px] mx-auto py-5 px-8 flex justify-between items-center sm:py-4 sm:px-5">
          <div class="font-serif text-2xl font-medium tracking-tight text-text-primary">Feed Preview</div>
        </div>
      </header>
        ...
        >
          ...
          >
            <div class="p-6 pb-4 flex justify-between items-center border-b border-border">
              <h2 class="font-serif text-xl font-medium m-0">Select Feeds to Import</h2>
              <button 
            ...
            <div class="p-6 overflow-y-auto max-h-[calc(85vh-180px)]">
              ...
              <div class="border-b border-border pb-4 mb-4">
                <label class="flex items-center gap-3 cursor-pointer group">
                  ...
                  <span class="font-medium">Select all ({{ sharedFeeds.length }} feeds)</span>
                </label>
              </div>
              ...
              <ul class="space-y-1 mb-4">
                <li 
                  ...
                  :class="selectedUrls.has(feed.url) ? 'bg-accent/5' : 'hover:bg-border/50'"
                >
                  <label class="flex items-center gap-3 p-3 cursor-pointer">
                    ...
                    <div class="flex-1 min-w-0" @click.prevent="toggleFeed(feed.url)">
                      <div class="font-medium truncate">{{ feed.name }}</div>
                      <div class="text-sm text-text-tertiary truncate">{{ feed.url }}</div>
                    </div>
                  </label>
                </li>
              </ul>
